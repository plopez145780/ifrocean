<html>
    <head>
        <meta charset="UTF-8">
        <title>Formulaire POST</title>
    </head>
    <body>
        <form method="post" action="08-POST.php">
            Nom : <input type="text" name="nom">
            <br>
            Latitude A :  <input type="text" name="Latitude A">
            <br>
			Longitude A :  <input type="text" name="Longitude A">
            <br>
			 Latitude B :  <input type="text" name="Latitude B">
            <br>
			Longitude B :  <input type="text" name="Longitude B">
            <br>
			 Latitude C :  <input type="text" name="Latitude C">
            <br>
			Longitude C :  <input type="text" name="Longitude C">
            <br>
			 Latitude D :  <input type="text" name="Latitude D ">
			 <br>
			 Longitude D :  <input type="text" name="Longitude D">
            <br>
			
            <input type="submit" value="Valider">
        </form>
    </body>
</html>